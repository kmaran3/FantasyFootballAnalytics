...
# Map nflreadpy team abbreviations to AVgrades abbreviations
# nflreadpy uses "LA" for LA Rams but AVgrades uses "LAR"
_TEAM_ABBR_MAP = {
    'LA': 'LAR',  # LA Rams
}
...
# ── Composite team grades from AVgrades pickle + nflreadpy live data ─────────
def _compute_composite_grades():
    global _avgrades_raw  # Store raw AVgrades for direct lookup
    per_team = {}
    _avgrades_raw = {}

    # Try to load AVgrades.pkl first
    avgrades_df = None
    avgrades_path = _PICKLE_DIR / 'AVgrades.pkl'
    if avgrades_path.exists():
        try:
            avgrades_df = pd.read_pickle(avgrades_path)
            print(f'Loaded AVgrades from {avgrades_path}')
        except Exception as e:
            print(f'Warning: could not load AVgrades.pkl: {e}')
    ...
    def _rank_normalize(vals_dict, invert=False):
        """Return rank (1-32) instead of percentile. Lower rank = better."""
        if not vals_dict:
            return {}
        sorted_vals = sorted(vals_dict.items(), key=lambda x: x[1], reverse=not invert)
        result = {}
        for rank, (team, val) in enumerate(sorted_vals, start=1):
            result[team] = rank  # 1 = best, 32 = worst
        return result

    # Use AVgrades if available; otherwise fall back to live computation
    use_avgrades = avgrades_df is not None and not avgrades_df.empty
    use_live = (not _nfl_latest.empty
                and '_pos' in _nfl_latest.columns
                and '_team' in _nfl_latest.columns
                and not use_avgrades)

    if use_avgrades:
        # Load grades from AVgrades.pkl
        latest_season = int(avgrades_df['season'].max())
        grades_df = avgrades_df[avgrades_df['season'] == latest_season].copy()
        grades_df['team'] = grades_df['team'].apply(_norm_team)

        # Normalize O-line grades: use rank scoring (1 = best, 32 = worst)
        for col in ['pass_oline', 'run_oline']:
            col_values = {team: val for team, val in zip(grades_df['team'], grades_df[col])}
            col_ranked = _rank_normalize(col_values, invert=False)
            grades_df[col] = grades_df['team'].map(col_ranked)
            print(f'  Normalized {col} to ranks (1 = best team, 32 = worst team)')

        # Store normalized AVgrades values for direct lookup
        for _, row in grades_df.iterrows():
            team = row['team']
            _avgrades_raw[team] = {
                'pass_oline': row['pass_oline'],  # Already normalized to rank (1-32)
                'run_oline': row['run_oline'],    # Already normalized to rank (1-32)
            }
        ...
        for _, row in grades_df.iterrows():
            ...
            per_team[team] = {
                ...
                'WR': round(row['wrte'], 1),
                'TE': round(row['wrte'], 1),
                'OLine_pass': row['pass_oline'],
                'OLine_run': row['run_oline'],
                'DST': round(row['dst'], 1),
            }
        print(f'  Loaded grades for {len(per_team)} teams from AVgrades')
        ...
        if fpc and gpc:
            ...
            for team in all_teams:
                ypc_norm = _minmax_normalize(ypc_by_team.to_dict()).get(team, 0.5)
                spg_norm = _minmax_normalize(spg_by_team.to_dict(), invert=True).get(team, 0.5)
                oline_run_raw[team] = ypc_norm  # Run blocking = RB rushing performance
                oline_pass_raw[team] = spg_norm  # Pass blocking inverse of sacks
            for team, v in oline_run_raw.items():
                per_team.setdefault(team, {})['OLine_run'] = round(v, 2)
            for team, v in oline_pass_raw.items():
                per_team.setdefault(team, {})['OLine_pass'] = round(v, 2)
        ...
        # Default OLine grades (will be normalized below)
        for team in per_team:
            per_team[team]['OLine_run'] = per_team[team].get('OLine_run', 0.5)
            per_team[team]['OLine_pass'] = per_team[team].get('OLine_pass', 0.5)

    # Ensure every team that appears in any grade has all 6 keys + both OLine variants (fill missing with median)
    all_teams = set(per_team.keys())
    print(f'  Teams with grades: {sorted(all_teams)}')
    
    for grade_key in ['QB', 'RB', 'WR', 'TE', 'OLine_run', 'OLine_pass', 'DST']:
    ...
    print(f'  Composite grades computed for {len(per_team)} teams')
    print(f'  Available team abbreviations in composite grades: {sorted(per_team.keys())}')
...
_avgrades_raw = {}  # Store raw AVgrades for direct lookup in player_quick_stats
...
def player_stats():
    ...
    team_grade = {}
    # Normalize team name to match keys in _composite_grades
    team_normalized = _norm_team(team) if team else None
    print(f'  Player: {name}, Team: {team}, Normalized: {team_normalized}')
    print(f'  Available teams in grades: {list(_composite_grades.keys()) if _composite_grades else "No grades"}')
    
    if team_normalized and team_normalized in _composite_grades:
        cg = _composite_grades[team_normalized]
        # Select OLine grade based on position: RBs get run_oline, others get pass_oline
        oline_grade = cg.get('OLine_run') if pos == 'RB' else cg.get('OLine_pass')
        team_grade = {
            'OLine': oline_grade,
            'QB':    cg.get('QB'),
            'RB':    cg.get('RB'),
        ...
        print(f'  WARNING: Team {team_normalized} not found in composite grades')
...
def player_quick_stats():
    ...
    # Get team grades from raw AVgrades (PFF grades not normalized)
    team_grade = {}
    team_normalized = _norm_team(team) if team else None
    if team_normalized and team_normalized in _avgrades_raw:
        grades = _avgrades_raw[team_normalized]
        # Return appropriate grade based on position:
        # RBs get run blocking grade, others (QB/WR/TE) get pass blocking grade
        if pos == 'RB':
            team_grade = {
                'grade_type': 'Run Blocking',
                'value': grades.get('run_oline'),
            }
        else:
            team_grade = {
                'grade_type': 'Pass Blocking',
                'value': grades.get('pass_oline'),
            }
